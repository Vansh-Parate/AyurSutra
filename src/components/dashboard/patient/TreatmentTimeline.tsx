...
const TreatmentTimeline: React.FC<TreatmentTimelineProps> = ({ totalDays, currentDay }) => {
  const items = Array.from({ length: totalDays }, (_, i) => i + 1)
  return (
    <div className="col-span-12 xl:col-span-7 rounded-xl border border-emerald-100 bg-white p-4">
      <div className="flex items-center justify-between">
        ...
        <span className="text-[12px] text-slate-600">Day 1 → Day {totalDays}</span>
      </div>
      <div className="mt-3">
        <div className="flex items-center gap-2 overflow-x-auto pb-1">
          <div className="flex items-center gap-2">
            {items.map(d => {
              const state = d < currentDay ? 'past' : d === currentDay ? 'today' : 'upcoming'
              const cls = state === 'past'
                ? 'bg-emerald-500 text-white border-emerald-600 hover:bg-emerald-600'
                : state === 'today'
                ? 'bg-amber-400 text-amber-900 border-amber-500 hover:bg-amber-500'
                : 'bg-slate-100 text-slate-700 border-slate-200 hover:bg-slate-200'
              return (
                <button key={d} className={`shrink-0 inline-flex items-center justify-center w-8 h-8 rounded-full border text-[12px] transition outline-offset-2 focus:outline focus:outline-2 focus:outline-emerald-300 ${cls}`}>{d}</button>
              )
            })}
          </div>
        </div>
